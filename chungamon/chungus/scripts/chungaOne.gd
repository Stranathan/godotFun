...
#	directionInput.y = Input.get_action_strength("player_move_down") - Input.get_action_strength("player_move_up")
	...
	#--  using d-pad
	directionInput.x = Input.get_action_strength("chunga_right") - Input.get_action_strength("chunga_left")
	directionInput.y = Input.get_action_strength("chunga_down") - Input.get_action_strength("chunga_up")
	directionInput = directionInput.normalized();
	
	# -------- Input Update --------
	if Input.is_action_just_pressed("chunga_attack"):
		attack();
	...
	if collision:
		move_and_collide(0.05 * MAX_SPEED * -directionInput)
